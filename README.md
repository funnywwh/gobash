...
# 如果没有指定作业ID，默认使用当前作业或最后一个作业
$ fg
$ bg
```

**注意**: Windows平台不支持 `Ctrl+Z` 信号处理，这是平台限制。其他作业控制功能（后台任务、jobs、fg、bg）在Windows上可以正常使用。
...
- [x] 扩展builtin命令测试（cat、ls、rm、rmdir、test、type、env、which、head、tail、wc、grep、sort、uniq、cut、clear等）
