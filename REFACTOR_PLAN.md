...
- [ ] 支持多字节字符（UTF-8）（需要进一步实现和测试）
  - [x] 基础框架已添加（readChar 使用 utf8.DecodeRuneInString，Lexer 结构已扩展）
  - [ ] 正确处理多字节字符的引号（readString 需要修复）
  - [ ] 正确处理多字节字符的变量名（NextToken 需要修复以支持多字节字符进入 default 分支）
  - [ ] 添加 UTF-8 支持测试用例
