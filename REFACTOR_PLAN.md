...
- [ ] 支持多字节字符（UTF-8）
  - [ ] 步骤 1: 修改 Lexer 结构体，添加 UTF-8 支持字段
    - [ ] 添加 `chRune rune` 字段存储当前字符的 rune 值
    - [ ] 添加 `chWidth int` 字段存储当前字符的字节宽度
    - [ ] 保留 `ch byte` 字段用于 ASCII 字符的快速比较
  - [ ] 步骤 2: 重构 readChar() 函数支持 UTF-8
    - [ ] 使用 `utf8.DecodeRuneInString` 读取 UTF-8 字符
    - [ ] 正确处理多字节字符的字节位置更新
    - [ ] 正确处理行号和列号的更新（多字节字符列号只增加 1）
    - [ ] 保持 ASCII 字符的快速路径（ch < 128）
  - [ ] 步骤 3: 添加 peekCharRune() 辅助函数
    - [ ] 实现查看下一个 rune 但不移动位置的功能
    - [ ] 用于多字节字符的预览
  - [ ] 步骤 4: 修改 readIdentifier() 支持 UTF-8
    - [ ] 使用 `unicode.IsLetter` 和 `unicode.IsDigit` 检查多字节字符
    - [ ] 正确处理多字节字符的标识符读取
  - [ ] 步骤 5: 修改 readString() 支持 UTF-8
    - [ ] 正确处理多字节字符的引号匹配
    - [ ] 正确处理多字节字符的字符串内容读取
    - [ ] 使用 `strings.Builder` 的 `WriteRune` 方法
  - [ ] 步骤 6: 修改 NextToken() 支持 UTF-8
    - [ ] 处理多字节字符进入 default 分支的逻辑
    - [ ] 正确处理多字节字符的变量名识别
    - [ ] 正确处理多字节字符的标识符识别
  - [ ] 步骤 7: 修改 readIdentifierOrPath() 支持 UTF-8
    - [ ] 正确处理多字节字符的路径读取
    - [ ] 正确处理多字节字符的分隔符检查
  - [ ] 步骤 8: 添加 UTF-8 支持测试用例
    - [ ] 测试中文变量名和字符串
    - [ ] 测试包含中文的引号字符串
    - [ ] 测试包含日文的变量名
    - [ ] 测试多字节字符的路径
